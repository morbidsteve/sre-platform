...
export function PodsPanel({ active, refreshKey }: PodsPanelProps) {
  ...
  const podStatusCounts = useMemo(() => {
    ...
    for (const p of pods) {
      ...
      if (p.status === 'CrashLoopBackOff') crashLooping++;
    }
    ...
    <div>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-text-primary">Pods</h3>
          <span className="text-xs text-text-dim">
            {pods.length} total, {podStatusCounts.running} running{podStatusCounts.crashLooping > 0 ? `, ${podStatusCounts.crashLooping} crash-looping` : ''}
          </span>
        </div>
